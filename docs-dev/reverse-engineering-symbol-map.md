...
| Address | Likely symbol | Current counterpart | Evidence | Confidence |
| --- | --- | --- | --- | --- |
| `0x10016750` | `ED_CallSpawn` | `src/game/g_spawn.c` | Checks item list first, then `spawns[]`, prints `ED_CallSpawn: NULL classname` and `%s doesn't have a spawn function`. Shipped `pack/maps/barracko.bsp` includes a typoed `info_teleporter_dest`; because this path only logs and leaves the edict resident, the stock `misc_teleporter` helper can still use that entity by `targetname` without a dedicated spawn-table alias. | High |
